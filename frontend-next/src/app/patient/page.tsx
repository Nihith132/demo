...
import { getFirebaseAuth } from "@/lib/firebase_client";
...
export default function PatientDashboardPage() {
  ...
  async function signInGoogle() {
    ...
    try {
      ...
      await apiFetch("/api/patient-auth/session", { method: "POST", token });
      await loadHistory();
    } catch (e) {
      const msg =
        e instanceof Error
  ...
  async function loadHistory() {
    ...
    try {
      const token = await getFirebaseToken();
      const rows = await apiFetch<HistoryRow[]>("/api/patient/history", { token });
      setHistory(rows);
    } catch (e) {
  ...
  async function submitIntake() {
    ...
    try {
      ...
      // department is determined async; patient can refresh and then list doctors
      await loadHistory();
    } catch (e) {
  ...
  async function book() {
    if (!readyToBook || !activePatientId) return;
    setError(null);
    try {
      const token = await getFirebaseToken();
      const payload = {
  ...
  return (
    <div className="grid gap-6">
      <Card title="Patient dashboard" desc="Firebase Google sign-in + patient history">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card title="New intake" desc="Upload a PDF and describe symptoms">
          <div className="grid gap-3">
            ...
            <Button onClick={submitIntake} disabled={!authed}>
              Submit intake
            </Button>
        ...
        <Card title="Booking" desc="Load doctors for the active case and book">
          <div className="grid gap-3">
            <Button variant="secondary" onClick={loadDoctors} disabled={!authed || !activePatientId}>
              Load doctors
            </Button>
            ...
            <Button onClick={book} disabled={!authed || !readyToBook}>
              Book appointment
            </Button>
          </div>
        </Card>
